...
export class EventsService {
  ...
  async createEventPaymentMonths(
    eventId: number,
    startMonth: string,
    termMonths: number,
  ) {
  ...
  async createEventChecklist(eventId: number, text: string, date: string) {
    ...
    if (!textTrim)
      throw new BadRequestException('Texto do item é obrigatório.');
  ...
  async deleteEventChecklistItem(id: number) {
    const idNumber = Number(id);
    if (Number.isNaN(idNumber))
      throw new BadRequestException('ID do item de checklist inválido.');
