/*
  ...
  - You are about to drop the `collection_participant` table. If the table is not empty, all the data it contains will be lost.
  - You are about to drop the `event_payment_months` table. If the table is not empty, all the data it contains will be lost.
...
-- DropForeignKey
ALTER TABLE `collection_participant` DROP FOREIGN KEY `collection_participant_groupId_fkey`;
...
-- DropTable
DROP TABLE `collection_participant`;

-- DropTable
DROP TABLE `event_payment_months`;
...
CREATE TABLE `participant` (
    ...
    INDEX `participant_eventId_groupId_idx`(`eventId`, `groupId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- CreateTable
CREATE TABLE `event_payment_month` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `eventId` INTEGER NOT NULL,
    `month` VARCHAR(7) NOT NULL,

    INDEX `event_payment_month_eventId_idx`(`eventId`),
    INDEX `event_payment_month_month_idx`(`month`),
    UNIQUE INDEX `event_payment_month_eventId_month_key`(`eventId`, `month`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `participant` ADD CONSTRAINT `participant_groupId_fkey` FOREIGN KEY (`groupId`) REFERENCES `event_group`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `event_payment_month` ADD CONSTRAINT `event_payment_month_eventId_fkey` FOREIGN KEY (`eventId`) REFERENCES `event`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
