...
import CreateFolderModal from "../events/storage/create-folder-modal";
import { usePathname, useRouter, useSearchParams } from "next/navigation";
import { apiFetch } from "@/lib/api";
import FolderCard from "../events/storage/folder-card";
import { ArrowLeft, ArrowRight, SquareArrowLeft, Upload } from "lucide-react";
import RenameNodeModal from "../events/storage/rename-files-modal";
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
      ...
      {(state.status === "done" || state.status === "error") && (
        <div
          className={`h-2 w-2 shrink-0 rounded-full ${state.status === "done" ? "bg-emerald-400" : "bg-rose-400"}`}
        />
      )}
    </div>
  );
          ...
          {(currentFolderId || externalCrumb) && (
            <>
              <button
                type="button"
                onClick={
                  currentFolderId ? handleGoBackFolder : externalCrumb?.onBack
                }
