...
export default function CollectionsTab({
  ...
  async function addParticipant() {
    ...
    try {
      await apiFetch("/events/participants", "POST", {
        eventId,
        name,
  ...
  async function fetchParticipants() {
    ...
    try {
      ...
      if (err instanceof Error) setError(err.message);
      else setError("Falha ao carregar participantes.");
      setParticipants([]);
    } finally {
      setLoading(false);
    }
  ...
  async function fetchCollections() {
    ...
    try {
      ...
      if (err instanceof Error) setError(err.message);
      else setError("Falha ao carregar recolhimentos.");
      setPayments({});
    } finally {
      setLoading(false);
    }
  ...
  async function fetchPaymentsMonths() {
    ...
    try {
      const data = await apiFetch(
        `/events/${eventId}/event_payment_months`,
        "GET",
      );
  ...
  function totalGeneral(months: { key: string }[]) {
    let total = 0;
    for (const m of months) {
    ...
    try {
      const canvas = await html2canvas(exportRef.current, {
        ...
        onclone: (doc) => {
          ...
          const headRow = mainTable.querySelector(
            "thead tr",
          ...
          const lastRowCells = mainTable.querySelectorAll(
          ...
          const lastColCells = mainTable.querySelectorAll(
          ...
          nodes.forEach((node) => {
            ...
            if (
              ...
              cs.borderRightColor.includes("lab(") ||
              cs.borderBottomColor.includes("lab(") ||
              cs.borderLeftColor.includes("lab(")
            ) {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(monthsToRender.length / MONTHS_PER_PAGE),
  );
  ...
  const shouldShowSetupScreen = monthsLoaded && paymentMonths.length === 0;
